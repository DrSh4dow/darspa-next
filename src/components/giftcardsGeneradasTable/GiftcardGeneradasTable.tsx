...
export default function GiftcardGeneradasTable() {
  const salesQuery = trpc.admin.getAllGeneratedSales.useQuery();
      ...
      <tbody>
        {salesQuery.data.map((s) => (
          <tr key={s.id} className="border-b bg-white  ">
            ...
            >
              {s.productPrismicName}
            </th>
            <td className="py-4 px-6">{s.createdAt.toLocaleString("es-CL")}</td>
            <td className="py-4 px-6">{formatter.format(s.total)}</td>
